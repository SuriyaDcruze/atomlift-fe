...
const ThisMonthOverdueScreen: React.FC<ThisMonthOverdueScreenProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    fetchThisMonthOverdue();
  }, []);

  const fetchThisMonthOverdue = async (): Promise<void> => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      console.error('Error fetching this month overdue services:', err);
